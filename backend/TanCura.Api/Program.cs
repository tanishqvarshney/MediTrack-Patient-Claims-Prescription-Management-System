...
using TanCura.Infrastructure.Repositories;
using TanCura.Infrastructure.Services;
using TanCura.Api.Middleware;
...
{
    c.SwaggerDoc("v1", new() { Title = "TanCura Claims API", Version = "v1" });
...
// Database
builder.Services.AddDbContext<TanCuraDbContext>(opts =>
    opts.UseSqlServer(
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(opts =>
    {
        opts.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            ValidateIssuer = true,
            ValidIssuer = "tancura-api",
            ValidateAudience = true,
            ValidAudience = "tancura-client",
...
builder.Services.AddCors(opts =>
    opts.AddPolicy("Angular", p => p
        .WithOrigins("http://localhost", "http://localhost:4200", "https://tancura.io")
        .AllowAnyHeader()
        .AllowAnyMethod()));
...
if (app.Environment.IsDevelopment())
{
    ...
    var db = scope.ServiceProvider.GetRequiredService<TanCuraDbContext>();
