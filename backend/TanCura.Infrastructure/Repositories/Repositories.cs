...
namespace TanCura.Infrastructure.Repositories;

public class ClaimRepository : IClaimRepository
{
    private readonly TanCuraDbContext _db;
    public ClaimRepository(TanCuraDbContext db) => _db = db;
...
{
    private readonly TanCuraDbContext _db;
    public FormularyRepository(TanCuraDbContext db) => _db = db;
    ...
    public async Task<List<Formulary>> SearchAsync(string query, CancellationToken ct = default) =>
        await _db.Formulary
            .Where(f => (f.DrugName.Contains(query) || f.NDCCode.Contains(query)) && f.IsActive)
            .Take(10)
            .ToListAsync(ct);
}
...
{
    private readonly TanCuraDbContext _db;
    public UserRepository(TanCuraDbContext db) => _db = db;
...
{
    private readonly TanCuraDbContext _db;
    public ProviderRepository(TanCuraDbContext db) => _db = db;
